  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (profileData && profileData.companyName) setBrandName(profileData.companyName);
        ...
        const affiliatesRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/shopify/affiliates/brand`, {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {isShopifyConnected === false ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-8 text-center max-w-2xl mx-auto mt-10">
          ...
          <p className="text-amber-700 mb-6">You need to connect your Shopify store to view analytics, revenue, and tracking data.</p>
          <button
